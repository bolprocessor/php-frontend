...
function convert_musicxml($the_score,$repeat_section,$divisions,$fifths,$mode,$midi_channel,$dynamic_control,$select_part,$ignore_dynamics,$tempo_option,$create_channels,$include_breaths,$include_slurs,$include_measures,$ignore_fermata,$ignore_mordents,$chromatic_mordents,$ignore_turns,$chromatic_turns,$ignore_trills,$chromatic_trills,$ignore_arpeggios,$reload_musicxml,$test_musicxml,$change_metronome_average,$change_metronome_min,$change_metronome_max,$current_metronome_average,$current_metronome_min,$current_metronome_max,$list_corrections,$trace_tempo,$trace_ornamentations,$breath_length,$breath_tag,$trace_measures,$measures,$accept_signs,$include_parts,$number_parts,$apply_rndtime,$rndtime,$apply_rndvel,$rndvel,$extend_last_measure,$number_measures,$accept_pedal) {
		...
		//  Beware that there are empty sessions
			foreach($the_section as $i_measure => $the_measure) {
				...
				if(!is_integer($i_measure)) {
					...
					if($list_this) $report .= "• Implicit measure #".$i_measure." is labelled <span class=\"turquoise-text\">“".$measure_label[$i_measure]."”</span><br />";
					}
				...
				if($include_measures /* AND !$first_measure */ AND $reload_musicxml) {
					$data .= " [—".$i_measure."—] ";
	//				echo "@@@<br />";
					}
				...
				foreach($the_measure as $score_part => $the_part) {
					...
					$note_on = $is_chord = $rest = $pitch = $unpitched = $time_modification = $forward = $backup = $chord_in_process = $dynamics = $upper_mordent = $lower_mordent = $trill = $turn = $sustainstart = $sustainstop = $sostenutostart = $sostenutostop = $softstart = $softstop = FALSE;
					...
					foreach($the_part as $i_line => $line) {
						...
						//		if($list_this) $report .= ">> chord p/q = ".($p_this_dur / $q_this_dur).", sum_durations = ".$sum_durations."<br />";
							...
							if($sustainstop AND $sustainstart) $curr_event[$score_part][$j]['sustain'] = "stopstart";
							else if($sustainstart) $curr_event[$score_part][$j]['sustain'] = "start";
							else if($sustainstop) $curr_event[$score_part][$j]['sustain'] = "stop";
							if($sostenutostop AND $sostenutostart) $curr_event[$score_part][$j]['sostenuto'] = "stopstart";
							else if($sostenutostart) $curr_event[$score_part][$j]['sostenuto'] = "start";
							else if($sostenutostop) $curr_event[$score_part][$j]['sostenuto'] = "stop";
							if($softstop AND $softstart) $curr_event[$score_part][$j]['soft'] = "stopstart";
							else if($softstart) $curr_event[$score_part][$j]['soft'] = "start";
							else if($softstop) $curr_event[$score_part][$j]['soft'] = "stop";
						...
						//		$time_this_field += $curr_event[$score_part][$j]['p_dur'] / $curr_event[$score_part][$j]['q_dur'];
							...
							$note_on = $rest = $fermata = $is_chord = $upper_mordent = $lower_mordent = $trill = $turn = $chromatic = $sustainstart = $sustainstop = $sostenutostart = $sostenutostop = $softstart = $softstop = FALSE;
					...
					//		echo "slur = ".$slur_type."<br />";
						...
						//		$more_duration = 0;
								...
								if($time <> $breath_location[$n_breath]) {
									$message = "<span class=\"red-text\">Error in breath location:</span> ".$time." ≠ ".$breath_location[$n_breath]." measure #".$i_measure."<br />";
					...
					///		$more_duration += $duration;
						...
						if(is_integer($pos=strpos($line,"<pedal"))) {
							...
							$pedal_type = (string)$this_xml['type'];
							$pedal_sign = (string)$this_xml['sign'];
							if ($pedal_sign === '') $pedal_sign = 'sustain';
							if($pedal_sign == "sustain" AND ($pedal_type == "start" OR $pedal_type == "resume")) $sustainstart = TRUE;
							if($pedal_sign == "sustain" AND ($pedal_type == "stop" OR $pedal_type == "discontinue")) $sustainstop = TRUE;
							if($pedal_sign == "sostenuto" AND ($pedal_type == "start" OR $pedal_type == "resume")) $sostenutostart = TRUE;
							if($pedal_sign == "sostenuto" AND ($pedal_type == "stop" OR $pedal_type == "discontinue")) $sostenutostop = TRUE;

							if($pedal_sign == "soft" AND ($pedal_type == "start" OR $pedal_type == "resume")) $softstart = TRUE;
							if($pedal_sign == "soft" AND ($pedal_type == "stop" OR $pedal_type == "discontinue")) $softstop = TRUE;
							}
					...
					foreach($curr_event[$score_part] as $j => $the_event) {
						if(!isset($the_event['type'])) { // Added by BB 2022-01-28
							if($list_this) $report .= "<span class=\"red-text\">Potential error:</span> the_event['type'] is not set: measure #".$i_measure." j = ".$j."<br />";
							continue;
							}
						if(isset($the_event['turn'])) {
							if($list_this) $report .= "<span class=\"green-text\">turn ";
							$turn_beats = $the_event['turn-beats'];
							$long_ornamentation = $the_event['long'];
							if($list_this) $report .= "beats = ".$turn_beats."</span> ";
						...
						if($the_event['type'] == "mm" AND $i_field_of_measure == 0) { // Fixed by BB 2022-02-01
							...
							if($p_time_field == 0) {
								...
								if($list_this OR $trace_tempo) $report .= "<span class=\"red-text\">mm</span> Measure #".$i_measure." part [".$score_part."] field #".($i_field_of_part + 1)." metronome set to <span class=\"green-text\">".$new_tempo."</span> at date ".$p_date_this_tempo/($q_date_this_tempo * $divisions[$score_part])." beat(s) [d]<br />";
								$current_period = 60 / $new_tempo;
								$new_tempo = 0;
								}
					//		$old_tempo = $current_tempo = $default_tempo[$section] = $fraction;
						...
						if($p_time_field == 0 AND $i_field_of_part == 0 AND $stream_units > 0 AND $p_stream_duration == 0 AND $new_tempo > 0) {
							...
							$current_period = 60 / $new_tempo;
							if($list_this OR $trace_tempo) $report .= "<span class=\"red-text\">mm</span> Measure #".$i_measure." part ".$score_part." field #".($i_field_of_part + 1)." metronome set to <span class=\"green-text\">".$new_tempo."</span> at date ".$fraction_date." beat(s) [a]<br />";
							$final_metronome = $new_tempo;
							$fraction = $new_tempo."/60";
							$simplify = simplify($fraction,$max_term_in_fraction);
							$fraction = $simplify['fraction'];
						...
						if($the_event['type'] == "back" OR ($p_stream_duration > 0 AND $new_tempo > 0)) {
							...
							if($i_field_of_part > 0 AND $new_tempo > 0) {
								$convert_measure[$score_part] .= "||".$new_tempo."||";
								$current_period = 60 / $new_tempo;
								if($list_this OR $trace_tempo) $report .= "<span class=\"red-text\">mm</span> Measure #".$i_measure." part ".$score_part." field #".($i_field_of_part + 1)." metronome set to <span class=\"green-text\">".$new_tempo."</span> at date ".$fraction_date." beat(s) [b]<br />";
								$final_metronome = $new_tempo;
								$new_tempo = 0;
								}
							if($test_musicxml)
							...
							if($stream_units > 0) {
								...
								if($new_tempo > 0) {
									...
									if($list_this OR $trace_tempo) $report .= "<span class=\"red-text\">mm</span> Measure #".$i_measure." part [".$score_part."] field #".($i_field_of_part + 1)." metronome set to <span class=\"green-text\">".$new_tempo."</span> at date ".$fraction_date_new_tempo." beat(s) [c]<br />";
									$final_metronome = $new_tempo;
								...
								if($new_tempo > 0) {
									$stream .= "||".$new_tempo."||";
									$current_period = 60 / $new_tempo;
									if($list_this OR $trace_tempo) $report .= "<span class=\"red-text\">mm</span> Measure #".$i_measure." part ".$score_part." field #".($i_field_of_part + 1)." metronome set to <span class=\"green-text\">".$new_tempo."</span> at date ".$fraction_date." beat(s) [d]<br />";
									$final_metronome = $new_tempo;
									if($p_time_field == 0 AND $i_field_of_part == 0) {
										$fraction = $new_tempo."/60";
										$simplify = simplify($fraction,$max_term_in_fraction);
							...
							if($the_event['type'] == "back") {
								...
								if(($p_rest * $q_rest) > 0 AND ($p_rest/$q_rest) > ($divisions[$score_part]/100)) {
									...
									$fraction = $simplify['fraction'];
									if(($p_rest * $q_rest) < 0) {
										if($list_this) $report .= "<span class=\"red-text\">➡ </span> Error in measure ".$i_measure." part ".$score_part." field #".($i_field_of_part + 1).", ‘backup’ rest = -".$fraction." beat(s) <span class=\"turquoise-text\">(fixed)</span><br />";
										}
									else if($p_rest > 0 AND ($p_rest/$q_rest) <= ($divisions[$score_part]/100)) {
										if($list_this) $report .= "<span class=\"turquoise-text\">• </span> Rounding part ".$score_part." measure ".$i_measure." field #".($i_field_of_part + 1).", neglecting ‘backup’ rest = ".$fraction."<br />";
										}
									$p_rest = 0; $q_rest = 1;
									$stream = ''; $stream_units = 0;
					...
					//		if($i_field_of_part == 0) $physical_time += $current_period * ($p_stream_duration / $q_stream_duration / $divisions[$score_part]);
							...
							$simplify_date = simplify($fraction_date,$max_term_in_fraction);
							$fraction_date = $simplify_date['fraction'];
							if($list_this) $report .= "<span class=\"turquoise-text\">f+ </span> Measure #".$i_measure." part ".$score_part." field #".($i_field_of_part + 1)." note ‘".$the_event['note']."’ at date ".$fraction_date." increased by ".$fraction ." beat(s) as per fermata #".($i_fermata + 1)."<br />";
							}
						if($i_field_of_part > 0 AND $p_date_next_fermata >= 0 AND ($starting_chord OR $the_event['type'] == "seq") AND $the_event['note'] <> '') {
							...
							if(($add['p'] * $add['q']) >= 0) {
								...
								$simplify_date = simplify($fraction_date,$max_term_in_fraction);
								$fraction_date = $simplify_date['fraction'];
								if($list_this) $report .= "<span class=\"turquoise-text\">f+ </span> Measure #".$i_measure." part ".$score_part." field #".($i_field_of_part + 1)." note ‘".$the_event['note']."’ at date ".$fraction_date." increased by ".$fraction." beat(s) to insert fermata #".($i_fermata + 1)."<br />";
								}
								...
								if(($add['p'] * $add['q']) >= 0) {
									...
									$simplify_date = simplify($fraction_date,$max_term_in_fraction);
									$fraction_date = $simplify_date['fraction'];
									if($list_this) $report .= "<span class=\"turquoise-text\">f+ </span> Measure #".$i_measure." part ".$score_part." field #".($i_field_of_part + 1)." silence at date ".$fraction_date." increased by ".$fraction." to insert fermata #".($i_fermata + 1)."<br />";
									$the_event['p_dur'] = $add['p'];
									$the_event['q_dur'] = $add['q'];
									}
								}
						...
						if(isset($the_event['breath_mark']) AND !$found_breath) {
							...
							if($list_this) $report .= "<span class=\"red-text\">Breath</span> measure #".$i_measure." at ".($p_breath / ($q_breath * $divisions[$score_part]))." beat(s)<br />";
							}
						if(isset($breath_location[$i_breath])) {
							$add = add($p_time_field,$q_time_field,$p_stream_duration,$q_stream_duration);
							$add = add($add['p'],$add['q'],-$breath_location[$i_breath],1);
						...
						if(!$is_chord) {
							...
							//		if($list_this) $report .= "p_time_field/q_time_field = ".$p_time_field."/".$q_time_field."<br />";
									...
									if($new_tempo > 0) { // maybe useless
										$convert_measure[$score_part] .= "||".$new_tempo."||";
										$current_period = 60 / $new_tempo;
										if($list_this OR $trace_tempo) $report .= "<span class=\"red-text\">mm</span> Measure #".$i_measure." part ".$score_part." field #".($i_field_of_part + 1)." metronome set to <span class=\"green-text\">".$new_tempo."</span> at date ".$fraction_date." beat(s) [f]<br />";
										$final_metronome = $new_tempo;
										if($p_time_field == 0 AND $i_field_of_part == 0) {
											$fraction = $new_tempo."/60";
											$simplify = simplify($fraction,$max_term_in_fraction);
										...
										//	if(($p_initial_part / $q_initial_part) > 0.05) {
											...
											//	if($list_this) $report .= "event_dur = ".$the_event['p_dur']."/".$the_event['q_dur']." initial_part = ".$p_initial_part."/".$q_initial_part."<br />";
												...
												if($new_tempo <> round(60 / $current_period)) { // Added by BB 2021-03-16
													...
													if($list_this OR $trace_tempo) $report .= "<span class=\"red-text\">mm</span> Measure #".$i_measure." part ".$score_part." field #".($i_field_of_part + 1)." metronome set to <span class=\"green-text\">".$new_tempo."</span> (".($i_new_tempo - 1)."th value) during rest at date ".$fraction_date_new_tempo."  beat(s)<br />";
													$final_metronome = $new_tempo;
													}
												$new_tempo = 0;
												}
							...
							if(isset($the_event['sustain']) AND $accept_pedal[$i_part - 1]) {
								// This is unlikely to happen: pedal command in upper notes of a chord
								if($the_event['sustain'] == "start") $convert_measure[$score_part] .= " _sustainstart_ ";
								else if($the_event['sustain'] == "stop") $convert_measure[$score_part] .= " _sustainstop_ ";
								else if($the_event['sustain'] == "stopstart") $convert_measure[$score_part] .= " _sustainstopstart_ ";
								}
							if(isset($the_event['sostenuto']) AND $accept_pedal[$i_part - 1]) {
								// This is unlikely to happen: pedal command in upper notes of a chord
								if($the_event['sostenuto'] == "start") $convert_measure[$score_part] .= " _sostenutostart_ ";
								else if($the_event['sostenuto'] == "stop") $convert_measure[$score_part] .= " _sostenutostop_ ";
								else if($the_event['sostenuto'] == "stopstart") $convert_measure[$score_part] .= " _sostenutostopstart_ ";
								}
							if(isset($the_event['soft']) AND $accept_pedal[$i_part - 1]) {
								// This is unlikely to happen: pedal command in upper notes of a chord
								if($the_event['soft'] == "start") $convert_measure[$score_part] .= " _softstart_ ";
								else if($the_event['soft'] == "stop") $convert_measure[$score_part] .= " _softstop_ ";
								else if($the_event['soft'] == "stopstart") $convert_measure[$score_part] .= " _softstopstart_ ";
								}
					...
					for($j = 0; $j < $i_field_of_part; $j++) {
						...
						if(($p_gap * $q_gap) > 0) {
								...
								if($j > 0 AND $p_next_date_new_tempo > 0) {
									...
									if(($add['p'] * $add['q']) >= 0) {
										...
										if($p_initial_part >= 0 OR (abs($p_initial_part / $q_initial_part / $divisions[$score_part]) < 0.05)) {
											...
											$stream .= " ||".$new_tempo."|| ";
											if($list_this OR $trace_tempo) $report .= "<span class=\"red-text\">mm</span> Measure #".$i_measure." part [".$score_part."] field #".($j + 1)." metronome set to <span class=\"green-text\">".$new_tempo."</span> (".$i_new_tempo_mem."th value) during additional rest at date ".$fraction_date_new_tempo." beat(s)<br />";
											$final_metronome = $new_tempo;
											$new_tempo = 0;
											}
										else break;
							...
							if($list_this OR $trace_tempo) $report .= "<span class=\"turquoise-text\">+rest </span> Measure #".$i_measure." part ".$score_part." field #".($j + 1)." added rest = ".$fraction." beat(s) (".$fraction_mem.")<br />";
							}
						}
			...
			//		if($list_this) $report .= "➡ Measure #".$i_measure." part [".$score_part."] physical time = ".round($max_physical_time,2)."s, average metronome of this measure = ".$metronome_this_measure.", final metronome = <span class=\"green-text\">".$final_metronome."</span><br />";
					if($list_this OR $trace_tempo) $report .= "➡ Measure #".$i_measure." part [".$score_part."] physical time = ".round($max_physical_time,2)."s, final metronome = <span class=\"green-text\">".$final_metronome."</span><br />";
	...
	if(isset($i_part)) {
		$data = str_replace("_sustainstart_","_switch_on_sustain_part(".$i_part.") ",$data);
		$data = str_replace("_sustainstop_","_switch_off_sustain_part(".$i_part.") ",$data);
		$data = str_replace("_sustainstopstart_","_switch_off_sustain_part(".$i_part.") _switch_on_sustain_part(".$i_part.") ",$data);
		$data = str_replace("_sostenutostart_","_switch_on_sostenuto_part(".$i_part.") ",$data);
		$data = str_replace("_sostenutostop_","_switch_off_sostenuto_part(".$i_part.") ",$data);
		$data = str_replace("_sostenutostopstart_","_switch_off_sostenuto_part(".$i_part.") _switch_on_sostenuto_part(".$i_part.") ",$data);
		$data = str_replace("_softstart_","_switch_on_soft_part(".$i_part.") ",$data);
		$data = str_replace("_softstop_","_switch_off_soft_part(".$i_part.") ",$data);
		$data = str_replace("_softstopstart_","_switch_off_soft_part(".$i_part.") _switch_on_soft_part(".$i_part.") ",$data);
		}
	...
	if($number_metronome_markers > 0)
		echo "<p><span class=\"red-text\">➡</span> ".$number_metronome_markers." metronome markers have been found inside measures of this score</p>";
	else
		echo "<p><span class=\"red-text\">➡</span> No metronome marker has been found inside measures of this score</p>";
...
function add_note($stream,$i_measure,$the_event,$long_ornamentation,$approach_ornamentation,$diatonic_scale_string,$ok_pedal,$last_notes) {
	...
	if(isset($the_event['sustain']) AND $ok_pedal) {
		if($the_event['sustain'] == "start") $stream .= " _sustainstart_ ";
		else if($the_event['sustain'] == "stop") $stream .= " _sustainstop_ ";
		else if($the_event['sustain'] == "stopstart") $stream .= " _sustainstopstart_ ";
		}
	if(isset($the_event['sostenuto']) AND $ok_pedal) {
		if($the_event['sostenuto'] == "start") $stream .= " _sostenutostart_ ";
		else if($the_event['sostenuto'] == "stop") $stream .= " _sostenutostop_ ";
		else if($the_event['sostenuto'] == "stopstart") $stream .= " _sostenutostopstart_ ";
		}
	if(isset($the_event['soft']) AND $ok_pedal) {
		if($the_event['soft'] == "start") $stream .= " _softstart_ ";
		else if($the_event['soft'] == "stop") $stream .= " _softstop_ ";
		else if($the_event['soft'] == "stopstart") $stream .= " _softstopstart_ ";
		}
...
function ornament($note,$long,$link,$diatonic_scale,$direction,$fifths,$trill,$trill_beats,$turn,$turn_beats,$chromatic,$trace_ornamentations) {
	...
	global $notes_diesis,$notes_bemol;
	$legato = $nolegato = $sustainstart = $sustainstop = $sustainstopstart = $sostenutostart = $sostenutostop = $sostenutostopstart = $softstart = $softstop = $softstopstart = FALSE;
	...
	if(is_integer(strpos($note,"_sustainstart_"))) {
		$sustainstart = TRUE;
		$note = trim(str_replace("_sustainstart_",'',$note));
		}
	if(is_integer(strpos($note,"_sustainstop_"))) {
		$sustainstop = TRUE;
		$note = trim(str_replace("_sustainstop_",'',$note));
		}
	if(is_integer(strpos($note,"_sustainstopstart_"))) {
		$sustainstop = TRUE;
		$note = trim(str_replace("_sustainstopstart_",'',$note));
		}
	if(is_integer(strpos($note,"_sostenutostart_"))) {
		$sostenutostart = TRUE;
		$note = trim(str_replace("_sostenutostart_",'',$note));
		}
	if(is_integer(strpos($note,"_sostenutostop_"))) {
		$sostenutostop = TRUE;
		$note = trim(str_replace("_sostenutostop_",'',$note));
		}
	if(is_integer(strpos($note,"_sostenutostopstart_"))) {
		$sostenutostop = TRUE;
		$note = trim(str_replace("_sostenutostopstart_",'',$note));
		}
	if(is_integer(strpos($note,"_softstart_"))) {
		$softstart = TRUE;
		$note = trim(str_replace("_softstart_",'',$note));
		}
	if(is_integer(strpos($note,"_softstop_"))) {
		$softstop = TRUE;
		$note = trim(str_replace("_softstop_",'',$note));
		}
	if(is_integer(strpos($note,"_softstopstart_"))) {
		$softstop = TRUE;
		$note = trim(str_replace("_softstopstart_",'',$note));
		}
	...
	if($pitch_class > 12) {
		echo "<span class=\"red-text\">➡</span> Incorrect pitch class for ".$note." in mordent ".$direction."<br />";
		$note2 = "???";
		}
	...
	if($chromatic) {
		...
		if($alt_note <> '' AND $pitch_class == $lower_pitch_class) echo "<span class=\"red-text\">➡</span> Error pitch class ".$pitch_class." not changed in ".$expression."<br />";
		$octave = preg_replace("/.+([0-9]+)/u","$1",$note);
		...
		if($alt_note <> '' AND $pitch_class == $higher_pitch_class) echo "<span class=\"red-text\">➡</span> Error pitch class ".$pitch_class." not changed in ".$expression."<br />";
	...
	if($sustainstart) $expression = " _sustainstart_ ".$expression;
	if($sustainstop) $expression = " _sustainstop_ ".$expression;
	if($sustainstopstart) $expression = " _sustainstopstart_ ".$expression;
	if($sostenutostart) $expression = " _sostenutostart_ ".$expression;
	if($sostenutostop) $expression = " _sostenutostop_ ".$expression;
	if($sostenutostopstart) $expression = " _sostenutostopstart_ ".$expression;
	if($softstart) $expression = " _softstart_ ".$expression;
	if($softstop) $expression = " _softstop_ ".$expression;
	if($softstopstart) $expression = " _softstopstart_ ".$expression;
	return $expression;
	}
...
//	return $data;
	...
	//	echo $p_duration."/".$q_duration." divisions = ".$divisions." divisions1 = ".$divisions1." divisions2 = ".$divisions2." ".$old_expression."<br />";
		...
		for($i = 0; $i < count($table); $i++) { // Fixed by BB 2022-02-10
			...
			$sustainstart = is_integer(strpos($note,"_sustainstart_"));
			$sustainstop = is_integer(strpos($note,"_sustainstop_"));
			$sustainstopstart = is_integer(strpos($note,"_sustainstopstart_"));
			$sostenutostart = is_integer(strpos($note,"_sostenutostart_"));
			$sostenutostop = is_integer(strpos($note,"_sostenutostop_"));
			$sostenutostopstart = is_integer(strpos($note,"_sostenutostopstart_"));
			$softstart = is_integer(strpos($note,"_softstart_"));
			$softstop = is_integer(strpos($note,"_softstop_"));
			$softstopstart = is_integer(strpos($note,"_softstopstart_"));
			$note = trim(str_replace("_legato_",'',$note));
			$note = trim(str_replace("_nolegato_",'',$note));
			$note = trim(str_replace("_sustainstart_",'',$note));
			$note = trim(str_replace("_sustainstop_",'',$note));
			...
			if($sustainstart) $new_expression1 .= "_sustainstart_ ";
			if($sustainstop) $new_expression1 .= "_sustainstop_ ";
			if($sustainstopstart) $new_expression1 .= "_sustainstopstart_ ";
			if($sostenutostart) $new_expression1 .= "_sostenutostart_ ";
			if($sostenutostop) $new_expression1 .= "_sostenutostop_ ";
			if($sostenutostopstart) $new_expression1 .= "_sostenutostopstart_ ";
			if($softstart) $new_expression1 .= "_softstart_ ";
			if($softstop) $new_expression1 .= "_softstop_ ";
			if($softstopstart) $new_expression1 .= "_softstopstart_ ";
